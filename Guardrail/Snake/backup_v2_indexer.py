...
if __name__ == "__main__":
    ...
    arcpy.CreateFileGDB_management(output, "GuardrailPoints")
    werkspace = output + os.sep + "GuardrailPoints.gdb"

    sample = "C:\\TxDOT\\Scripts\\Guardrail\\Data\\GuardrailPoints.gdb\\GuardrailPoints"
    sr = arcpy.SpatialReference(3857)
    arcpy.CreateFeatureclass_management(werkspace, "GuardrailPoints", "POINT", sample, "DISABLED", "DISABLED", sr)
    fc = werkspace + os.sep + "GuardrailPoints"
    print "dbase created."
    flds = ["SHAPE@"]
    ...
    counter = 0
    while qmin <= cValue:
        theReturn = fl.query(where="OBJECTID > " + str(qmin) + "and OBJECTID <= " + str(qmax),out_fields='*',returnGeometry=True)
